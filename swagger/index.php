...
<head>
  <meta charset="UTF-8">
  <title>Описание API — Барыбинцы</title>
  <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
  <link rel="stylesheet" type="text/css" href="<?php $_SERVER['DOCUMENT_ROOT']; ?>/swagger/swagger-ui.css" />
  <link rel="shortcut icon" href="<?php $_SERVER['DOCUMENT_ROOT']; ?>/res/logo_black.png">
  ...
  <style>
    ...
    *:after {
    ...
    .swagger-ui .topbar {
      background: #c80680;
      background: -moz-linear-gradient(left, #c80680 0%, #731472 41%);
      background: -webkit-linear-gradient(left, #c80680 0%, #731472 41%);
      background: -ms-linear-gradient(left, #c80680 0%, #731472 41%);
      background: linear-gradient(to right, #c80680 0%, #731472 41%);
    }
    ...
    .swagger-ui .response-col_description>.response-col_description__inner>.renderedMarkdown>p {
      margin: unset;
    }
    ...
    .swagger-ui .renderedMarkdown:nth-child(2) {
      font-size: 10pt;
    }
    ...
    .swagger-ui tr[data-code='200'] {
      background-color: rgba(29, 129, 39, 0.07);
    }

    .swagger-ui tr[data-code='401'],
    .swagger-ui tr[data-code='412'] {
  ...
  <?php include $_SERVER['DOCUMENT_ROOT'] . '/../legacy.barybians.ru/includes/styles.php' ?>
...
<body>
  ...
  <footer>
    <div>Powered by Swagger</div>
  </footer>
  <script src="<?php $_SERVER['DOCUMENT_ROOT']; ?>/swagger/swagger-ui-bundle.js" charset="UTF-8"> </script>
  <script src="<?php $_SERVER['DOCUMENT_ROOT']; ?>/swagger/swagger-ui-standalone-preset.js" charset="UTF-8"> </script>
  <script src="<?php $_SERVER['DOCUMENT_ROOT']; ?>/swagger/func.js" charset="UTF-8"> </script>
  <script>
  ...
  <script src="<?php $_SERVER['DOCUMENT_ROOT']; ?>/swagger/jquery-3.6.3.min.js"></script>
  <script>
    $('body').click(function() {
      dima();
    });
    ...
    function dima() {
      $('.url').remove();
      $("a.nostyle").each(function(index) {

        var dima = $(this).html().replace(/\{(.+?)\}/g, '<span style="color:red">{</span><span style="color:green">$1</span><span style="color:red">}</span>');
        $(this).html(dima);

        var dima = $(this).html().replace('LongPoll', '<span style="color:red">LongPoll</span>');
        $(this).html(dima);

        if (detectMobile()) {

          if ($(this).parent().parent().children('.authorization__btn')) {
            $(this).parent().parent().children('.authorization__btn').insertAfter($(this).parent().parent().children('.opblock-summary-path'))
          }

          dima = $(this).html().replace('/api', '');
          $(this).html(dima);
          dima = $(this).html().replace('messageId', 'msgId');
          $(this).html(dima);
        }

      });

      $("div.opblock-summary-description").each(function(index) {
        if ($(this).html().match('LongPoll')) {
          var dima = $(this).html().replace('LongPoll', '');
          $(this).html(dima);
          if (detectMobile()) {
            $('<button class="authorization__btn unlocked longpoll"><span class="long_poll" style="font-weight: bold" data-toggle="tooltip" title="Long Poll">LP</span></button>').insertAfter($(this).parent().children('.opblock-summary-path'));
          } else {
            $('<button class="authorization__btn unlocked longpoll"><span class="long_poll" style="font-weight: bold" data-toggle="tooltip" title="Long Poll">LP</span></button>').insertAfter(this);

          }
        }
      });
      $('input[placeholder="request - Random generated uuid"').val(uuidv4());
      $('.opblock-summary-path').on('click', 'span', function(e) {
        //console.log($(this).parent('.opblock-summary-path').data('path'))
        var copy = '';
        copy = $(this).text();
        copy = decodeURIComponent(copy);
        copy = 'https://barybians.ru' + copy;
